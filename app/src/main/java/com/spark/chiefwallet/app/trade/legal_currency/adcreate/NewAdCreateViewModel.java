...
import com.spark.chiefwallet.app.trade.legal_currency.adcreate.upload.AdUploadViewModel;
...
public class NewAdCreateViewModel extends BaseViewModel {
    public NewAdCreateViewModel(@NonNull Application application) {
        super(application);
    }
    ...
    private CurrencyTypeBean currencyResult;
    private List<Integer> payTypeList = new ArrayList<>();
    private String[] typedata = new String[]{App.getInstance().getResources().getString(R.string.buy2), App.getInstance().getResources().getString(R.string.sell2)};
    private String[] currencyType;

    public ObservableField<String> typecoinName = new ObservableField<>(App.getInstance().getResources().getString(R.string.buy2));
    ...
    public ObservableField<String> premiumPrice = new ObservableField<>("100");
    ...
    public ObservableField<String> tradePrice = new ObservableField<>("0.0");
    ...
    private ChoosePaymentMethodPopup choosePaymentMethodPopup;                     //侧拉栏
    ...
    public class UIChangeObservable {
        ...
        public SingleLiveEvent<Integer> adType = new SingleLiveEvent<>();
    ...
    public void getTradeCoinList() {
        ...
        if (ads != null) {
            AdvertiseScanClient.getInstance().priceFind(ads.getCoinName(), ads.getLocalCurrency(), adType);
        }
    }

    //选择创建类型
    public BindingCommand typecoinNameCoinCommand = new BindingCommand(new BindingAction() {
        @Override
        public void call() {
            new XPopup.Builder(mContext)
                    .asBottomList(App.getInstance().getString(R.string.type2), typedata,
                        ...
                        .asBottomList(App.getInstance().getString(R.string.str_slect_country), coinAddressArray,
                                new OnSelectListener() {
                                    ...
                                    public void onSelect(int position, String text) {
                                        ...
                                        AdvertiseScanClient.getInstance().priceFind(coinName.get(), tradeAreaListResult.getData().get(position).getLocalCurrency(), adType);
                                    }
                                })
                        .show();
            } else {
...
//                new XPopup.Builder(mContext)
//                        .asBottomList(App.getInstance().getString(R.string.str_slect_country), currencyType,
//                                new OnSelectListener() {
//                                    @Override
//                                    public void onSelect(int position, String text) {
//                                        //拿到货币类型
//                                        coinType.set(currencyResult.getData().get(position));
//                                        AdvertiseScanClient.getInstance().priceFind(coinName.get(), currencyResult.getData().get(position), adType);
//                                        currencyType.clone();
//                                    }
//                                })
//                        .show();
//            } else {
            if (TextUtils.isEmpty(coinAddressStr)) return;
            showDialog(App.getInstance().getString(R.string.str_requesting));
            AdvertiseScanClient.getInstance().findNationalCurrency(coinAddressStr);
        ...
        switch (eventBean.getOrigin()) {
            ...
            case EvKey.payList:
                if (eventBean.isStatue()) {
                    ...
                    PayListBean payListBean = (PayListBean) eventBean.getObject();
...
//                    if (choosePaymentMethodPopup == null)
                    ...
                    new XPopup.Builder(mContext)
                            ...
                            .asCustom(choosePaymentMethodPopup)
                            .show();
                } else {
            ...
            case EvKey.tradeAreaList:
                ...
                break;
            case EvKey.findAdcurrency:
                dismissDialog();
                if (eventBean.isStatue()) {
                    currencyResult = (CurrencyTypeBean) eventBean.getObject();
                    currencyType = new String[currencyResult.getData().size()];
                    for (int i = 0; i < currencyResult.getData().size(); i++) {
                        currencyType[i] = currencyResult.getData().get(i);
                    }
                    new XPopup.Builder(mContext)
                            .asBottomList(App.getInstance().getString(R.string.str_coin_type2), currencyType,
                                    new OnSelectListener() {
                                        @Override
                                        public void onSelect(int position, String text) {
                                            coinType.set(currencyResult.getData().get(position));
    ...
    private void updatePrice(FindPriceResult findPriceResult) {
        ...
        fixedPriceTips.set(App.getInstance().getString(R.string.str_macket_price) + ratePrice + coinType.get() + "\n" + App.getInstance().getString(R.string.str_ad_float_rate_tip));
        premiumPriceTips.set(App.getInstance().getString(R.string.str_macket_price) + ratePrice + coinType.get() + "\n" + App.getInstance().getString(R.string.str_ad_fixed_price_tip));
        if (isFixedPrice.get()) {
