...
import com.spark.klinelib.base.IAdapter;
import com.spark.klinelib.base.IChartDraw;
import com.spark.klinelib.base.IDateTimeFormatter;
import com.spark.klinelib.base.IValueFormatter;
import com.spark.klinelib.draw.MainDraw;
import com.spark.klinelib.draw.Status;
import com.spark.klinelib.entity.IKLine;
import com.spark.klinelib.formatter.TimeFormatter;
import com.spark.klinelib.formatter.ValueFormatter;
import com.spark.klinelib.utils.ViewUtil;
...
public abstract class BaseKLineChartView extends ScrollAndScaleView {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        this.mWidth = w - ViewUtil.Dp2Px(context, 42);
        this.mWidths = w;
    ...
    private void initRect() {
        if (isShowChild) {
            ...
            mMainRect = new Rect(0, mTopPadding, mWidths, mTopPadding + mMainHeight);
    ...
    private void drawText(Canvas canvas) {
        ...
        //--------------画上方k线图的值-------------
        if (mMainDraw != null) {
            canvas.drawText(formatValueByCoinScale(mMainMaxValue), mWidths - calculateWidth(formatValueByCoinScale(mMainMaxValue)), baseLine + mMainRect.top, mTextPaint);
            if (mMainLowMinValue <= 0) {
                canvas.drawText(formatValueByCoinScale(0L), mWidths - calculateWidth(formatValueByCoinScale(mMainMinValue)), mMainRect.bottom - textHeight + baseLine, mTextPaint);
            } else {
                canvas.drawText(formatValueByCoinScale(mMainMinValue), mWidths - calculateWidth(formatValueByCoinScale(mMainMinValue)), mMainRect.bottom - textHeight + baseLine, mTextPaint);
            }
            ...
            for (int i = 1; i < mGridRows; i++) {
                ...
                canvas.drawText(text, mWidths - calculateWidth(text), fixTextY(rowSpace * i + mMainRect.top), mTextPaint);
            }
        }
        //--------------画中间子图的值-------------
        if (mVolDraw != null) {
            canvas.drawText(mVolDraw.getValueFormatter().format(mVolMaxValue),
                    mWidths - calculateWidth(formatValue(mVolMaxValue)), mMainRect.bottom + baseLine, mTextPaint);
        ...
        if (mChildDraw != null) {
            canvas.drawText(mChildDraw.getValueFormatter().format(mChildMaxValue),
                    mWidths - calculateWidth(formatValue(mChildMaxValue)), mVolRect.bottom + baseLine, mTextPaint);
